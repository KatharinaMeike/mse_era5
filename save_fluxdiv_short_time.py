# Compute the MSE advection, MSE fluxes and their divergences from ERA5 data and save them to a netCDF file
#%%
...
timesteps = pd.date_range(start='2005-06-03 4:00', end='2005-06-03 6:00', freq='1h')
...
for timestep in timesteps:
    ...
    # MSE tendency due to shortwave radiation
    mse_tend_sw_levlat = np.nanmean(bud.mse_tend_sw, axis=-1) # Zonal mean
    mse_tend_sw_latlon = bud.compute_vertical_integral(bud.mse_tend_sw)
    levlat_output.append(bud.to_xarray_levlat(mse_tend_sw_levlat, var_name='mse_tendency_shortwave'))
    latlon_output.append(bud.to_xarray_latlon(mse_tend_sw_latlon, var_name='mse_tendency_shortwave'))

    # MSE tendency due to longwave radiation
    mse_tend_lw_levlat = np.nanmean(bud.mse_tend_lw, axis=-1) # Zonal mean
    mse_tend_lw_latlon = bud.compute_vertical_integral(bud.mse_tend_lw)
    levlat_output.append(bud.to_xarray_levlat(mse_tend_lw_levlat, var_name='mse_tendency_longwave'))
    latlon_output.append(bud.to_xarray_latlon(mse_tend_lw_latlon, var_name='mse_tendency_longwave'))

    # MSE tendency due to convection and turbulence
    mse_tend_conv_levlat = np.nanmean(bud.mse_tend_conv, axis=-1) # Zonal mean
    mse_tend_conv_latlon = bud.compute_vertical_integral(bud.mse_tend_conv)
    levlat_output.append(bud.to_xarray_levlat(mse_tend_conv_levlat, var_name='mse_tendency_convection'))
    latlon_output.append(bud.to_xarray_latlon(mse_tend_conv_latlon, var_name='mse_tendency_convection'))

    # Vertical integral of MSE tendency due to shortwave radiation
    mse_flux_sw = bud.compute_vertical_integral_to_p(bud.mse_tend_sw)
    mse_flux_sw_levlat = np.nanmean(mse_flux_sw, axis=-1) # Zonal mean
    mse_flux_sw_latlon = bud.compute_vertical_integral(mse_flux_sw)
    levlat_output.append(bud.to_xarray_levlat(mse_flux_sw_levlat, var_name='shortwave_mse_flux'))
    latlon_output.append(bud.to_xarray_latlon(mse_flux_sw_latlon, var_name='shortwave_mse_flux'))
    mse_flux_sw = None

    # Vertical integral of MSE tendency due to longwave radiation
    mse_flux_lw = bud.compute_vertical_integral_to_p(bud.mse_tend_lw)
    mse_flux_lw_levlat = np.nanmean(mse_flux_lw, axis=-1) # Zonal mean
    mse_flux_lw_latlon = bud.compute_vertical_integral(mse_flux_lw)
    levlat_output.append(bud.to_xarray_levlat(mse_flux_lw_levlat, var_name='longwave_mse_flux'))
    latlon_output.append(bud.to_xarray_latlon(mse_flux_lw_latlon, var_name='longwave_mse_flux'))
    mse_flux_lw = None

    # Vertical integral of MSE tendency due to convection and turbulence
    mse_flux_conv = bud.compute_vertical_integral_to_p(bud.mse_tend_conv)
    mse_flux_conv_levlat = np.nanmean(mse_flux_conv, axis=-1) # Zonal mean
    mse_flux_conv_latlon = bud.compute_vertical_integral(mse_flux_conv)
    levlat_output.append(bud.to_xarray_levlat(mse_flux_conv_levlat, var_name='convection_mse_flux'))
    latlon_output.append(bud.to_xarray_latlon(mse_flux_conv_latlon, var_name='convection_mse_flux'))
    mse_flux_conv = None

    output_filename = f"mse_tendency_levlat.nc"
    xr.merge(levlat_output).to_netcdf(output_path + output_filename + timestep.strftime('_%Y%m%d%H') + '.nc')

    output_filename = f"mse_tendency_latlon.nc"
